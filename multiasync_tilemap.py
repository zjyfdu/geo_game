...
def create_image_url_v2(minlon, minlat, maxlon, maxlat, minzoom, maxzoom, basetileurl, rootpath):
    ...
    for zoom in range(minzoom, maxzoom + 1):
        ...
        # 有些情况会算出来是-1或者1，强制改到0
        mintms_x = 0
        maxtms_y = 0
        maxtms_x = 2 ** zoom - 1
        mintms_y = 2 ** zoom - 1
        print('mintile', 'X:', mintms_x, 'Y:', mintms_y, 'zoom:', zoom)
        print('maxtile', 'Y:', maxtms_x, 'Y:', maxtms_y, 'zoom:', zoom)

        create_image_path(rootpath, zoom)

        for x in range(mintms_x, maxtms_x + 1):
            for y in range(mintms_x, maxtms_x + 1):
                savepath = './%s/%d/%d_%d.png'%(rootpath, zoom, x, maxtms_x - y)
                # tileurl = basetileurl + '&x=%d&y=%d&z=%d'%(x, y, zoom)
                tileurl = basetileurl % (zoom, x, y)
...
def main():
    minlon, minlat = -180, 85 # 矩形区域左下角坐标
    maxlon, maxlat = 180, -85 # 矩形区域右上角坐标
    minzoom, maxzoom = 9, 9

    # basetileurl = 'http://webrd02.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=8'

    # 腾讯地图的路网
    # basetileurl = 'https://rt0.map.gtimg.com/tile?type=vector&styleid=3&version=392'

    # 腾讯地图的地形图
    basetileurl = 'https://p3.map.gtimg.com/demTiles/%d/3/2/%d_%d.jpg'
